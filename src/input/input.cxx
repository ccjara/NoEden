...
Input::Input(Events* events) : events_(events) {
    assert(events_);
}
...
void Input::set_mouse_position(Vec2<i32> pos) {
    mouse_.move(pos);
    events_->engine->trigger<MouseMoveEvent>(pos);
}

void Input::set_mouse_button_pressed(MouseButton button, bool pressed) {
    if (pressed) {
        mouse_.mouse_down(button);
        events_->engine->trigger<MouseDownEvent>(button);
    } else {
        mouse_.mouse_up(button);
        events_->engine->trigger<MouseUpEvent>(button);
    }
}
...
void Input::set_key_pressed(Key key, bool pressed) {
    if (pressed) {
        ...
        events_->engine->trigger<KeyDownEvent>(key);
    } else {
        keyboard_.key_up(key);
        events_->engine->trigger<KeyUpEvent>(key);
    }
}
