...
std::unordered_map<std::string, std::unique_ptr<Material>> parse_materials(
    ...
    for (const auto& [file, table] : material_files) {
    ...
    for (const auto& material_id : sort_result.sorted_entities) {
        ...
        const auto root_iter = material_files.find(material->source);
        if (root_iter == material_files.cend()) {
        ...
        if (thermal_properties.is_object()) {
            material->thermal_properties.ignites_at = static_cast<i32> (
                thermal_properties["ignites_at"].as_number().value_or(0)
            );
        }
    }
